...
@st.cache_resource
def load_model():
    """Load the trained YOLO and Faster R-CNN models"""
    ...
    try:
        ...
        # Load Faster R-CNN model from FasterRCNN_ResNet50_Optimized for validation
        frcnn_path = "C:/Users/anish/OneDrive/Desktop/FYP/fyp-palm/runs/detect/FasterRCNN_ResNet50_Optimized/weights/best.pt"
        
        # Create Faster R-CNN model (3 classes: background, healthy, unhealthy)
...
def validate_with_faster_rcnn(detections, image, faster_rcnn_model, conf_threshold=0.15):
    """
    VALIDATION: Validate YOLO detections with Faster R-CNN from FasterRCNN_ResNet50_Optimized
    
    Purpose: Ensure YOLO detected real palm trees, not trash/false positives
    - Crop each YOLO detection
    - Run Faster R-CNN on crop to validate it's a tree
    - Keep only detections where Faster R-CNN also detects a tree
    - Reject detections where Faster R-CNN finds nothing (likely trash)
    
    OPTIMIZED: Smaller image size (128px) for faster processing
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # FIRST: Filter out rectangular detections
    ...
    # BATCH PROCESSING: Prepare all crops at once for faster inference
    crops_to_validate = []
    detection_indices = []
    ...
    for idx, detection in enumerate(detections):
        ...
        # Crop the detection from original image
        crop = image.crop((x1, y1, x2, y2))
        
        # FASTER: Use smaller size (128 instead of 224) for faster processing
        target_size = 128
        crop_resized = crop.resize((target_size, target_size), Image.BILINEAR)
        crop_tensor = TF.to_tensor(crop_resized).to(device)
        
        crops_to_validate.append(crop_tensor)
        detection_indices.append(idx)
    
    if len(crops_to_validate) == 0:
        return [], 0
    
    # BATCH INFERENCE: Process all crops at once (much faster than one-by-one)
    with torch.no_grad():
        batch_predictions = faster_rcnn_model(crops_to_validate)
    
    # Validate each prediction
    for batch_idx, (pred_idx, frcnn_pred) in enumerate(zip(detection_indices, batch_predictions)):
        detection = detections[pred_idx]
        
        # Check if Faster R-CNN detected anything
        if 'boxes' not in frcnn_pred or len(frcnn_pred['boxes']) == 0:
            # NO DETECTION = Trash/False positive, REJECT
            rejected_count += 1
            continue
        ...
        # VALIDATION CRITERIA: Faster R-CNN must detect a tree with decent confidence
        if top_score < conf_threshold:
            # Low confidence = Not a real tree, REJECT
            rejected_count += 1
            continue

        # Additional validation: Check if detection occupies reasonable portion of crop
        x1, y1, x2, y2 = top_box
        box_area = max(0.0, (x2 - x1) * (y2 - y1))
        crop_area = 128 * 128
        area_ratio = box_area / crop_area

        # Tree should occupy significant portion of crop
        if area_ratio < 0.05 or area_ratio > 0.98:
            # Detection too small or too large, REJECT
            rejected_count += 1
            continue

        # VALIDATION PASSED: Faster R-CNN confirms this is a real tree
        detection['cls'] = int(labels[top_idx] - 1) if labels is not None else detection.get('cls', 0)
        detection['conf'] = top_score  # Use Faster R-CNN confidence
        detection['validated'] = True
        validated_detections.append(detection)

    # Show validation results to user (ONLY if rejections happened)
    if rejected_count > 0:
        st.info(f"🔍 Validated {len(validated_detections)} palms (rejected {rejected_count} false positives)")


    return validated_detections, len(validated_detections)
...
def auto_detect_palms(model, image, confidence_threshold=0.05, validation_model=None):
    ...
    if initial_results and len(initial_results) > 0 and initial_results[0].boxes is not None:
        ...
        if avg_box_ratio > 0.05 or num_detections < 15:  # Even lower threshold + higher detection count requirement
            ...
            st.info("🔍 Aerial plantation detected - using individual tree analysis...")
            
            detections = tile_based_detection(model, image, confidence_threshold, tile_size=256)
            merged_detections = merge_tiled_detections(detections, image.size, iou_threshold=0.05)
            
            # Validate with Faster R-CNN if available
            if validation_model is not None and merged_detections:
                validated_detections, validated_count = validate_with_faster_rcnn(
                    merged_detections, image, validation_model, conf_threshold=0.15
                )
                merged_detections = validated_detections
            ...
            # Validate with Faster R-CNN if available
            if validation_model is not None and initial_results and initial_results[0].boxes is not None:
                ...
                # Validate with Faster R-CNN
                validated_detections, validated_count = validate_with_faster_rcnn(
                    detections, image, validation_model, conf_threshold=0.15
                )
                ...
                if validated_detections:
                    ...
                    result = MockResult(validated_detections, (image.height, image.width))
                    st.success(f"✅ Detection validated: {len(validated_detections)} palm trees confirmed!")
                    return [result]
...
def main():
    ...
    # Load models (YOLO and Faster R-CNN for validation)
    model, validation_model, model_path = load_model()
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            with st.expander(f" {uploaded_file.name}", expanded=True):
                ...
                with st.spinner("Analyzing palm health with optimized detection..."):
                    # Automatically use the best detection pipeline with Faster R-CNN validation
                    results = auto_detect_palms(model, img, confidence_threshold=0.05, validation_model=validation_model)
