...
def load_model():
    ...
    try:
        ...
        # SPEED OPTIMIZATION: Use half precision on GPU for 2x faster inference
        if torch.cuda.is_available():
            try:
                frcnn_model = frcnn_model.half()
                st.sidebar.info("🚀 GPU acceleration enabled with FP16 (2x faster)")
            except:
                pass  # If half precision fails, use full precision
...
def validate_image_quality(image, min_resolution=400, max_resolution=8000):
    ...
    if width < min_resolution or height < min_resolution:
        return False, f"Image too small ({width}x{height}). Minimum: {min_resolution}x{min_resolution}px"
    
    if width > max_resolution or height > max_resolution:
        return False, f"Image too large ({width}x{height}). Maximum: {max_resolution}x{max_resolution}px"
    ...
    try:
        ...
        if laplacian_var < 50:  # Very blurry
            return False, f"Image is too blurry (sharpness score: {laplacian_var:.1f})"
    except:
    ...
    try:
        ...
        if img_arr.std() < 20:  # Very low contrast
            return False, "Image has insufficient contrast"
    except:
        pass
    
    # Check if image is mostly uniform (satellite images that are too zoomed out)
    try:
        img_arr = np.array(image.convert('RGB'))
        ...
        green_ratio = green_mask.sum() / (255.0 * img_arr.shape[0] * img_arr.shape[1])
        
        # If image is almost entirely green (>85%), it might be too zoomed out
        if green_ratio > 0.85:
            return False, "Image appears to be too zoomed out (uniform vegetation)"
        
        # If image has very little green (<5%), not a palm plantation
        if green_ratio < 0.05:
            return False, "Image does not appear to contain palm trees"
...
def detect_with_faster_rcnn(faster_rcnn_model, image, confidence_threshold=0.5):
    """
    Detect palms using Faster R-CNN directly (no YOLO)
    Includes NMS to remove duplicate/overlapping detections
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # Use half precision if model supports it
    if next(faster_rcnn_model.parameters()).dtype == torch.float16:
    ...
    if len(boxes) == 0:
        st.warning("⚠️ Faster R-CNN: No palms detected at this confidence level.")
        return [MockResult([])]
    
    # Apply NMS (Non-Maximum Suppression) to remove overlapping detections
    # For single palm tree images: Use moderate NMS to merge overlapping frond detections into one tree
    nms_iou_threshold = 0.5  # Moderate: Merge boxes that overlap >50%
    ...
    before_nms = len(keep_conf.nonzero())
    after_nms = len(boxes)
    if before_nms > after_nms:
        st.info(f"🔍 NMS removed {before_nms - after_nms} overlapping detection(s). Keeping {after_nms} unique palms.")
    ...
    for i in range(len(boxes)):
        detections.append({
            'bbox': boxes[i],
            'conf': float(scores[i]),
            'cls': int(labels[i]) - 1  # Subtract 1 because Faster R-CNN uses 0 for background
        })
    
    # Filter rectangular detections
    before_filter = len(detections)
    detections = filter_rectangular_detections(detections, max_aspect_ratio=3.0, min_aspect_ratio=0.33)
    after_filter = len(detections)
    
    if before_filter > after_filter:
        filtered_count = before_filter - after_filter
        st.info(f"🔍 Filtered out {filtered_count} rectangular detection(s). Keeping {after_filter} circular/square palm crowns.")
    
    # Filter out non-vegetation (ground, shadows, etc.) - keep all green areas
    palm_detections = []
    non_palm_filtered = 0
    ...
    for detection in detections:
        ...
        # Crop detection region for basic validation
        crop = image.crop((x1, y1, x2, y2))
        crop_array = np.array(crop)
        
        # Convert to HSV for color analysis
        hsv = cv2.cvtColor(crop_array, cv2.COLOR_RGB2HSV)
        
        # Check green presence - must have reasonable green vegetation
        green_mask = cv2.inRange(hsv, (25, 30, 30), (90, 255, 255))
        green_ratio = np.sum(green_mask > 0) / green_mask.size
        
        # Check brightness - exclude very dark areas (ground, deep shadows)
        mean_brightness = np.mean(hsv[:, :, 2])
        
        # Simple vegetation check: Has green, reasonably bright
        is_vegetation = (green_ratio > 0.15) and (mean_brightness > 35)
        
        if is_vegetation:
            palm_detections.append(detection)
        else:
            non_palm_filtered += 1
    
    if non_palm_filtered > 0:
        st.info(f"🌳 Filtered out {non_palm_filtered} non-vegetation detection(s). Keeping {len(palm_detections)} palm detections.")
    
    detections = palm_detections
    
    # If multiple detections remain, keep only the largest/most confident one
    # This handles single palm tree images where we want just ONE detection
    if len(detections) > 1:
        ...
        detections = [detections_with_area[0][0]]
        st.info(f"🌴 Multiple detections merged into 1 palm tree (largest/most prominent).")
    
    # SECOND NMS: Apply very aggressive NMS again after aspect ratio filtering
    # This handles cases where multiple detections passed both NMS and aspect ratio filter
    if len(detections) > 1:
        det_boxes = torch.tensor([d['bbox'] for d in detections])
        det_scores = torch.tensor([d['conf'] for d in detections])
        
        # ULTRA AGGRESSIVE NMS: if boxes overlap at all (even 5%), keep only highest confidence
        aggressive_nms_threshold = 0.05
        keep_aggressive = nms(det_boxes, det_scores, aggressive_nms_threshold)
        
        before_aggressive = len(detections)
        detections = [detections[i] for i in keep_aggressive.numpy()]
        after_aggressive = len(detections)
        
        if before_aggressive > after_aggressive:
            st.info(f"🔍 Aggressive NMS removed {before_aggressive - after_aggressive} more overlapping detection(s). Final count: {after_aggressive} palms.")
...
def auto_detect_palms(model, image, confidence_threshold=0.05, validation_model=None):
    """
    SIMPLE 2-STEP: YOLO detection → Faster R-CNN validation → Aspect ratio filtering
    - Step 1: YOLO detects all palms (fast, single pass)
    - Step 2: Filter out rectangular detections (keep only circular/square palm crowns)
    - Step 3: Faster R-CNN validates to confirm individual tree crowns (automatic, transparent)
    """
    ...
    status_text.text("🔍 Step 1/3: Running YOLO detection...")
    ...
    if use_tiling:
        status_text.text("📐 Using tile-based detection for large/dense image...")
    ...
    # Filter out rectangular detections (keep only square/circular palm crowns)
    status_text.text("🔲 Step 2/3: Filtering rectangular detections...")
    before_filter_count = len(detections)
    detections = filter_rectangular_detections(detections, max_aspect_ratio=3.0, min_aspect_ratio=0.33)
    after_filter_count = len(detections)
    ...
    if before_filter_count > after_filter_count:
        ...
        st.info(f"🔍 Filtered out {filtered_count} rectangular detection(s). Keeping {after_filter_count} circular/square palm crowns.")

    # If a validation model is provided, validate detections
    validated_crops_count = 0
    if validation_model is not None and detections:
        status_text.text(f"✅ Step 3/3: Validating {len(detections)} detections with Faster R-CNN...")
        progress_bar.progress(75)
        
        before_validation_count = len(detections)
        validated, validated_crops_count = validate_with_faster_rcnn_2batch(detections, image, validation_model, progress_bar, status_text)
        detections = validated
        after_validation_count = len(detections)
        
        progress_bar.progress(95)
        
        # Show validation info to user
        rejected_count = before_validation_count - after_validation_count
        if rejected_count > 0:
            st.info(f"✅ Faster R-CNN validated {after_validation_count} individual palm crops. Rejected {rejected_count} non-palm detection(s).")
        else:
            st.info(f"✅ Faster R-CNN validated {after_validation_count} individual palm crops.")
    else:
        progress_bar.progress(95)
...
def filter_rectangular_detections(detections, max_aspect_ratio=3.0, min_aspect_ratio=0.33):
    """
    Filter out detections that are too rectangular (not circular/square palm crowns).
    Palm crowns should be roughly circular, so aspect ratio should be close to 1.0
    
    Args:
        detections: List of detection dicts with 'bbox' key
        max_aspect_ratio: Maximum width/height ratio (default 3.0 = EXTREMELY PERMISSIVE, 3x wider than tall)
        min_aspect_ratio: Minimum width/height ratio (default 0.33 = EXTREMELY PERMISSIVE, 3x taller than wide)
    
    Returns:
        Filtered list of detections with aspect ratio info
    """
    filtered = []
    ...
    for detection in detections:
        ...
        # Keep only detections with aspect ratio close to 1 (square/circular)
        # Reject very rectangular detections
        if min_aspect_ratio <= aspect_ratio <= max_aspect_ratio:
            filtered.append(detection)
        else:
            # Mark as filtered
            detection['filtered'] = True
            detection['filter_reason'] = f'Aspect ratio {aspect_ratio:.2f} outside range [{min_aspect_ratio:.2f}, {max_aspect_ratio:.2f}]'
    
    return filtered

def validate_with_faster_rcnn_2batch(detections, image, faster_rcnn_model, progress_bar=None, status_text=None, conf_threshold=0.01):
    """
    ...
    - Filter out rectangular detections (keep only square/circular palm crowns)
    - EXTREMELY PERMISSIVE: Minimal threshold (0.01), accepts nearly everything
    
    Returns:
        tuple: (validated_detections, validated_crops_count)
            - validated_detections: List of detections that passed validation
            - validated_crops_count: Number of individual palm crops validated by Faster R-CNN
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # FIRST: Filter out rectangular detections (like satellite/zoomed out images)
    detections = filter_rectangular_detections(detections, max_aspect_ratio=3.0, min_aspect_ratio=0.33)
    ...
    validated_crops_count = 0  # Track number of crops validated by Faster R-CNN
    
    # FASTER: Split into 4 smaller batches for parallel processing efficiency
    # Smaller batches = better GPU utilization and faster overall processing
    num_batches = min(4, len(detections))  # Use 4 batches or fewer if not enough detections
    batch_size = len(detections) // num_batches if num_batches > 0 else 0
    
    batches = []
    for i in range(num_batches):
        start_idx = i * batch_size
        end_idx = start_idx + batch_size if i < num_batches - 1 else len(detections)
        batches.append(detections[start_idx:end_idx])

    def process_batch(batch_items, batch_tensors, batch_num):
        """Run Faster R-CNN on a batch of crop tensors and return validated detections."""
        if status_text:
            status_text.text(f"🔬 Validating batch {batch_num}/2 ({len(batch_items)} crops)...")
        ...
        # FASTER: Use inference_mode instead of no_grad for better performance
        with torch.inference_mode():
            frcnn_preds = faster_rcnn_model(batch_tensors)

        for detection, frcnn_pred in zip(batch_items, frcnn_preds):
            # Basic sanity - if Faster R-CNN found nothing, check YOLO confidence
            if 'boxes' not in frcnn_pred or len(frcnn_pred['boxes']) == 0:
                # If YOLO was confident (≥0.3), trust YOLO and accept it anyway
                if detection.get('conf', 0) >= 0.3:
            ...
            # MAXIMUM PERMISSIVE: Trust YOLO, only reject obvious trash
            # - If Faster R-CNN finds ANYTHING, accept it
            # - If Faster R-CNN finds nothing but YOLO confidence was high, still accept
            is_valid = False
            
            # If Faster R-CNN detected ANYTHING at all (even with 0.01 confidence), ACCEPT
            if len(scores) > 0:
                is_valid = True
            
            # Even if Faster R-CNN found nothing, if YOLO was confident, trust YOLO
            if not is_valid and detection.get('conf', 0) >= 0.3:
                is_valid = True
    ...
    def build_batch_from_items(batch_list):
        ...
        for detection in batch_list:
            ...
            # PREPROCESSING: Enhance saturation to better differentiate green vs yellow palms
            # Convert to numpy array for color enhancement
            crop_array = np.array(crop)
            
            # Convert to HSV color space
            hsv = cv2.cvtColor(crop_array, cv2.COLOR_RGB2HSV).astype(np.float32)
            
            # Increase saturation by 30% to make colors more distinct
            hsv[:, :, 1] = np.clip(hsv[:, :, 1] * 1.3, 0, 255)
            
            # Slight increase in value/brightness for better visibility
            hsv[:, :, 2] = np.clip(hsv[:, :, 2] * 1.1, 0, 255)
            
            # Convert back to RGB
            hsv = hsv.astype(np.uint8)
            enhanced_crop = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)
            crop_enhanced = Image.fromarray(enhanced_crop)
            
            # FASTER: Smaller crop size (128x128) for much faster processing
            target_size = 128
            crop_resized = crop_enhanced.resize((target_size, target_size), Image.BILINEAR)
            crop_tensor = TF.to_tensor(crop_resized).to(device)
            
            # Use half precision if model is in half precision (GPU only)
            if next(faster_rcnn_model.parameters()).dtype == torch.float16:
    ...
    # Build and process all batches
    for batch_idx, batch in enumerate(batches):
        if len(batch) > 0:
            if progress_bar:
                progress = 78 + (batch_idx * 10 // num_batches)
                progress_bar.progress(progress)
            
            batch_items, batch_tensors = build_batch_from_items(batch)
            batch_validated = process_batch(batch_items, batch_tensors, batch_idx + 1)
            validated_detections.extend(batch_validated)
            validated_crops_count += len(batch_validated)

    return validated_detections, validated_crops_count
...
def show_upload_page():
    ...
    # Model selection
    col_model, col_conf = st.columns([2, 1])
    with col_model:
        detection_model = st.selectbox(
            "🤖 Select Detection Model",
            ["YOLO (Fast)", "Faster R-CNN (Accurate)", "YOLO + Faster R-CNN (Both)"],
            index=2,
            help="YOLO is faster, Faster R-CNN is more accurate, Both uses YOLO then validates with Faster R-CNN"
        )
    with col_conf:
    ...
    <div class="info-box">
        ...
        <ul>
            <li>✅ Upload aerial/drone images of palm plantations</li>
            <li>✅ System will automatically detect palm trees</li>
            <li>🔍 Rectangular detections are filtered out (keeps only circular/square palm crowns)</li>
            <li>🎯 Best results with clear individual palm crowns visible</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    uploaded_files = st.file_uploader(
        ...
        help="Supported formats: JPG, JPEG, PNG. Rectangular detections will be automatically filtered.",
    ...
    if uploaded_files:
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            with st.expander(f"🌴 {uploaded_file.name}", expanded=(idx == 0)):
                ...
                # Detect based on selected model
                with st.spinner(f"🤖 Analyzing with {detection_model}..."):
                    if detection_model == "YOLO (Fast)":
                        # YOLO only, no validation
                        results = auto_detect_palms(yolo_model, img, confidence_threshold=confidence_threshold, validation_model=None)
                    elif detection_model == "Faster R-CNN (Accurate)":
                        # Faster R-CNN only
                        results = detect_with_faster_rcnn(frcnn_model, img, confidence_threshold=confidence_threshold)
                    else:  # "YOLO + Faster R-CNN (Both)"
                        # YOLO detection + Faster R-CNN validation
                        results = auto_detect_palms(yolo_model, img, confidence_threshold=confidence_threshold, validation_model=frcnn_model)
