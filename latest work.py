...
def detect_with_faster_rcnn(faster_rcnn_model, image, confidence_threshold=0.5):
    ...
    detections = filter_rectangular_detections(detections, max_aspect_ratio=1.3, min_aspect_ratio=0.77)
    after_filter = len(detections)
    
    if before_filter > after_filter:
        filtered_count = before_filter - after_filter
        st.info(f"🔍 Faster R-CNN: Filtered out {filtered_count} rectangular detection(s). Keeping {after_filter} circular/square palm crowns.")
...
def auto_detect_palms(model, image, confidence_threshold=0.05, validation_model=None):
    ...
    if validation_model is not None and detections:
        ...
        validated, validated_crops_count = validate_with_faster_rcnn_2batch(detections, image, validation_model)
...
def filter_rectangular_detections(detections, max_aspect_ratio=1.3, min_aspect_ratio=0.77):
    """
    ...
    Args:
        ...
        max_aspect_ratio: Maximum width/height ratio (default 1.3 = 30% wider than tall)
        min_aspect_ratio: Minimum width/height ratio (default 0.77 = reciprocal of 1.3)
...
def validate_with_faster_rcnn_2batch(detections, image, faster_rcnn_model, conf_threshold=0.15):
    ...
    # Validate ALL detections with Faster R-CNN (no auto-keep based on confidence)
    # Split all detections into 2 batches for processing
    mid_point = len(detections) // 2
    batch1 = detections[:mid_point] if mid_point > 0 else []
    batch2 = detections[mid_point:] if mid_point < len(detections) else []
    ...
    if len(batch1) > 0:
        ...
        batch1_validated = process_batch(batch1_items, batch1_tensors)
        validated_detections.extend(batch1_validated)
        validated_crops_count += len(batch1_validated)

    # Process batch 2
    if len(batch2) > 0:
        batch2_items, batch2_tensors = build_batch_from_items(batch2)
        batch2_validated = process_batch(batch2_items, batch2_tensors)
        validated_detections.extend(batch2_validated)
        validated_crops_count += len(batch2_validated)
