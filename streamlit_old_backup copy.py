...
def load_faster_rcnn():
    """Load Faster R-CNN model for palm health classification"""
    try:
        import torchvision
        ...
        model_path = "C:/Users/anish/OneDrive/Desktop/FYP/fyp-palm/runs/detect/FasterRCNN_ResNet50_Optimized/weights/best.pt"
        
        # Create model architecture
        model = fasterrcnn_resnet50_fpn(pretrained=False)
        num_classes = 3  # Background + 2 classes (Healthy, Unhealthy)
        ...
        try:
            ...
            st.warning(f"⚠️ Faster R-CNN not available: {e}. Using YOLO classification.")
            return None, None

def classify_with_faster_rcnn(faster_rcnn_model, cropped_image):
    """Classify individual palm using Faster R-CNN"""
    try:
        ...
        # Resize to 224x224 for Faster R-CNN
        cropped_image_resized = cropped_image.resize((224, 224), Image.Resampling.LANCZOS)
        
        # Convert to tensor
        transform = transforms.Compose([
            transforms.ToTensor(),
        ])
        image_tensor = transform(cropped_image_resized).unsqueeze(0)
        ...
        if len(pred['boxes']) > 0:
            ...
            # Class mapping: 1 = Unhealthy (PalmAnom), 2 = Healthy (PalmSan)
            # Convert to YOLO format: 0 = Unhealthy, 1 = Healthy
            if predicted_class == 2:  # Healthy
                return 1, confidence
            else:  # Unhealthy (class 1) or Background (class 0)
                return 0, confidence
        else:
            # No detection - default to unhealthy with low confidence
            return 0, 0.3
...
def auto_detect_palms(model, image, confidence_threshold=0.05):
    ...
    if initial_results and len(initial_results) > 0 and initial_results[0].boxes is not None:
        ...
        if avg_box_ratio > 0.05 or num_detections < 15:  # Even lower threshold + higher detection count requirement
            ...
            # MERGE OVERLAPPING BOXES (50% total combined overlap threshold)
            merged_detections = merge_overlapping_boxes(merged_detections, overlap_threshold=0.5)
            ...
            if initial_results and len(initial_results) > 0 and initial_results[0].boxes is not None:
                ...
                # Merge overlapping boxes - remove if 50% covered by other boxes combined
                merged_detections = merge_overlapping_boxes(detections, overlap_threshold=0.5)
...
def create_detection_summary(results, image=None, faster_rcnn_model=None):
    """Create a detailed summary of detection results with Faster R-CNN classification"""
    ...
    for i in range(len(boxes)):
        ...
        # Classify with Faster R-CNN if available
        if faster_rcnn_model is not None and img_array is not None:
            # Crop palm region
            ...
            cls, conf = classify_with_faster_rcnn(faster_rcnn_model, cropped_palm)
        else:
            # Fallback to YOLO classification
            cls = int(boxes.cls[i])
            conf = float(boxes.conf[i])
...
def crop_individual_palms(image, results, faster_rcnn_model=None):
    """
    Crop individual palm trees from the image based on detection results
    Classify each palm with Faster R-CNN if available
...
def main():
    ...
    # Load Faster R-CNN for classification
    faster_rcnn_model, faster_rcnn_path = load_faster_rcnn()
    if faster_rcnn_model is not None:
        st.success("✅ Faster R-CNN classifier loaded for individual palm health analysis")
    else:
        st.info("ℹ️ Using YOLO classification (Faster R-CNN not available)")
    ...
    if uploaded_files:
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            with st.expander(f" {uploaded_file.name}", expanded=True):
                ...
                # Create summary with Faster R-CNN classification
                summary = create_detection_summary(results, img, faster_rcnn_model)
